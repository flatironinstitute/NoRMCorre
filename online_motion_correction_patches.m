function [M_final,shifts,template] = online_motion_correction_patches(Y,options,template)
    ...
    for t = 1:T
        if mod(t,bin_width) == 0
            ...
            if strcmpi(method{2},'mean')
                new_temp = cellfun(@(x) mean(x,nd+1), buffer, 'UniformOutput',false);
            elseif strcmpi(method{2},'median');
                new_temp = cellfun(@(x) median(x,nd+1), buffer, 'UniformOutput', false);
            end
            ...
            elseif strcmpi(method{1},'median');
                ...
                template = mat2cell_ov(median(buffer_med,nd+1),xx_s,xx_f,yy_s,yy_f,zz_s,zz_f,overlap_pre,sizY);
            end
            fftTemp = cellfun(@fftn, template, 'un',0);
            ...
            for i = 1:length(xx_s)
                for j = 1:length(yy_s)           
                    for k = 1:length(zz_s)
                        ...
                        if mot_uf == 1
                            M_fin{i,j,k} = remove_boundaries(M_temp,output(3:end),'zero',template{i,j,k},add_value);
                        end                                               
                    end
                end
            end            
            ...
            for i = 1:length(xx_uf)
                for j = 1:length(yy_uf)
                    for k = 1:length(zz_uf)
                        ...
                        M_fin{i,j,k} = shift_reconstruct(I_temp,shifts_up(i,j,k,:),us_fac,Nr{i,j,k},Nc{i,j,k},Np{i,j,k},'zero',add_value);
                    end
                end
            end
        else
