...
tic; [M1,shifts1,template1] = online_motion_correction_patches(Y,options,template1); toc

%% now try non-rigid
<<<<<<< HEAD:demo_laptop.m
options.grid_size = [32,128];                % size of patch in each direction
options.mot_uf = 4;                         % further upsample by a given factor
tic; [M3,shifts3,template3] = online_motion_correction_patches(Y,options); toc
=======
options.grid_size = [16,16];                % size of patch in each direction
options.plot_flag = false;                  % flag for plotting results while correcting
>>>>>>> origin/master:demo.m
...
[cM3,mM3,vM3] = motion_metrics(M3,15);
>>>>>>> origin/master:demo.m
T = length(cY);
%% plot metrics
figure;
...
=======
    subplot(211); plot(shifts_r(:,1),'--k','linewidth',2); hold all; plot(shifts_x);
        title('Displacements along the x axis')
        legend([{'rigid'},str{:}])
    subplot(212); plot(shifts_r(:,2),'--k','linewidth',2); hold all; plot(shifts_y)
        title('Displacements along the y axis')
%% plot movies
...
>>>>>>> origin/master:demo.m
    ...
    subplot(121);imagesc(M3(:,:,t),[nnY,mmY]); xlabel('non-rigid corrected','fontsize',14,'fontweight','bold'); axis equal; axis tight;
    title(sprintf('Frame %i out of %i',t,T),'fontweight','bold','fontsize',14); colormap('bone')
    subplot(122);imagesc(M4(:,:,t),[nnY,mmY]); xlabel('non-rigid corrected','fontsize',14,'fontweight','bold'); axis equal; axis tight;
    title(sprintf('Frame %i out of %i',t,T),'fontweight','bold','fontsize',14); colormap('bone')
    set(gca,'XTick',[],'YTick',[]);
    drawnow;
    pause(0.02);
