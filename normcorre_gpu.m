function [M_final,shifts_g,template,options,col_shift] = normcorre_gpu(Y,options,template)
...
for it = 1:iter
    for t = 1:bin_width:T
        ...
        %Ytm_gpu = gpuArray(Ytm);
        %Ytm_fft = fft(fft(Ytm_gpu,[],1),[],2);
        %if nd == 3
        %    Ytm_fft = fft(Ytm_fft,[],3);
        %end
        %minY = zeros(lY,1);
        %maxY = zeros(lY,1);
        Ytm_gpu = struct('data',cell(lY,1));
        for i = 1:lY
            Ytm_gpu(i).data = gpuArray(Ytm(otherdims{:},i));
        end
        minY = zeros(lY, 1, 'gpuArray');
        maxY = zeros(lY, 1, 'gpuArray');
        parfor i = 1:lY
           [shifts_i, min_i, max_i] = find_shifts_gpu(Ytm_gpu(i).data,fftTempMat,fftTemp,patches,options);
           minY(i) = min_i;
           maxY(i) = max_i;
           shifts(i) = gather(shifts_i);
        end
        %find_shifts = @(Y) find_shifts_gpu(Y.data,fftTempMat,fftTemp,patches,options);
        %[shifts,minY,maxY] = find_shifts_gpu(Yt,fftTempMat,fftTemp,patches,options);
        %[shifts,minY,maxY] = arrayfun(find_shifts, Ytm_gpu);
        ...
        for i = lY:-1:1
            Yt = Ytm(otherdims{:},i);
            %Yt = Ytm_gpu(i).data;
            future_results(i) = parfeval(@reconstruct_frame, 1, Yt, shifts(i).shifts, minY(i), maxY(i), options);
        end
        for i = 1:lY
            [idx, Mf_temp] = fetchNext(future_results);
